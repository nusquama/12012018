...
Class StyleGenerator
{
    ...
    public function generate_preview_style()
    {
        ...
        {
            $fields = Customizer::getInstance()->get_outputs();
            ...
            foreach($fields as $setting => $styles)
            {
                ...
                if(!empty($new_style))
                {
                    $style_tag .= "<style id='jnews-customizer-postmessage_{$setting}'>" . $new_style . "</style>\n";
                }
            }
        ...
        foreach($fields as $setting => $styles)
        {
            ...
            if($this->get_active_callback()->evaluate_by_id($setting))
            {
                foreach ($styles as $style)
                {
                    ...
                    if(!empty($new_style))
                    {
                        $generated_style[$style['element']][] = $new_style;
                    }
                }
            }
        }
