...
Class Typography extends ControlAbstract {
	...
	public function enqueue() {
		wp_enqueue_script( 'jnews-typography', JEG_URL . '/assets/js/customizer-control/control-typography.js', array( 'jquery', 'customize-base', 'selectize', 'wp-color-picker-alpha' ), null, true );
		do_action('jnews_load_all_font');
	}
	...
	public function to_json() {
		...
		$fields = Customizer::getInstance()->get_fields();
		$field = $fields[$this->id];
		$this->json['transport'] = $field['transport'];
	}
	...
	protected function content_template() {
		...
		<div class="wrapper">
			...
			<# if ( false !== data.default['color'] ) { #>
				<div class="color">
					<h5>Color</h5>
					<input type="text" data-palette="{{ data.palette }}" data-default-color="{{ data.default['color'] }}" value="{{ data.value['color'] }}" class="jnews-color-control color-picker" {{{ data.link }}} />
				</div>
			<# } #>
		</div>
		<?php
